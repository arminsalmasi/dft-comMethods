...
Nmax = 400; % number of mesh points
h = (rmax - rmin)/(Nmax-1);
...
load('r2R2Anal.txt'); % so run script 2 by 200 points otherwise lenghts of vectors are not equal
%% Armin: from script 2 :  r^2 * phi^2 = r^2 * n(r) norm hydrogen
for i = 2:Nmax-1
   r2DensityInit(i) = 2 * r2R2Anal(i); %* r(i); % a hydrogen 1s orbital, suitably normalized.
end
...
% Boundary conditions 
%% Armin: from charge integration on a surface of an sphear with a large enough radious 
BC0 = 0; BCNmax = q;
...
while (ediff >=convcrit)   % iteratate until convergence
    ...
    %% #Armin :  bH is VH'' = the Hartree potential
    % knowing that d^2U(r)/dr^2 = (r(i)vH)''= -4.Pi.r.n(r)
    for i = 2:Nmax-1
        bH(i-1) = h^2*(-4)*pi*r2DensityOld(i) / r(i); % this is equal to *r(i): there is an r^2 term in the r2n variable
    end
    ...
    %% #Armin: End of Hartree potential calculation
    %% Armin: Veff = VH + Vxc + Vext
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% Exchange term %%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Vxc = exc +n.(dexc /dn) = Vx +Vc = ex + n.(dex /dn) +ec + n.(dec /dn)
    for i = 1:Nmax
    ...
    %%#Armin: calculating ec here
    for i = 1:Nmax
        DensityOld(i) = r2DensityOld (i)/(r(i)*r(i));
        if DensityOld(i) < 0.0001
            Vc(i) = 0; ec(i) = 0;
        else
            %%#Armin: n = 3/(4.pi.rs^3) => rs =(3/4.pi.n(r))^(1/3)
            rs = (3 / (4 * pi * DensityOld(i)))^(1/3); % ## ## # fill in here ## ## #
            if rs >= 1,
    ...
    r2DensityOld = r2Density;% ## ## # fill in here ## ## #
